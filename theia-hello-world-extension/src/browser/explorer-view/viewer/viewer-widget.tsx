...
export const ViewerWidgetOptions = Symbol('ViewerWidgetOptions');
...
export class ViewerWidget extends ReactWidget {
  ...
  private onExperimentSelected = (experiment: any): void => this.doHandleExperimentSelectedSignal(experiment);
  ...
  @inject(ExplorerContribution) protected readonly traceExplorerContribution: ExplorerContribution;
  ...
  async init(): Promise<void> {
    ...
    if (this.options.traceUUID) {
      ...
      signalManager().fireExperimentOpenedSignal(1);
      this.update();
    }
  ...
  async initialize(): Promise<void> {
    ...
    console.info('viewer initialize');
  ...
  onActivateRequest(msg: Message): void {
    ...
    if (this.openedExperiment) {
      signalManager().fireTraceViewerTabActivatedSignal(this.openedExperiment);
      signalManager().fireExperimentSelectedSignal(1);
      signalManager().fireOpenedTracesChangedSignal(1);
    }
    this.node.focus();
    console.error('onActivateRequest');
  }
  ...
  protected render(): React.ReactNode {
    ...
    return <div className="viewer-container">{this.openedExperiment ? <div>Content</div> : 'Trace is loading...'}</div>;
  }
  ...
  protected doHandleExperimentSelectedSignal(experiment: any): void {
    // if (this.openedExperiment && this.openedExperiment.UUID === experiment.UUID) {
    if (this.openedExperiment === experiment) {
      this.shell.activateWidget(String(this.openedExperiment));
    }
  }
