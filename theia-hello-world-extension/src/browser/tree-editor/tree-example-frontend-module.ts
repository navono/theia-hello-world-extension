...
} from './example-file/example-file-contribution';
import { bindSamplePreferences } from './preferences/sample-preference-contribution';

/* eslint-disable */
import '../../../src/browser/tree-editor/style/editor.css';

export const bindTreeEditor = (bind: interfaces.Bind) => {
  ...
  bind(WidgetFactory).toDynamicValue(({ container }) => ({
    ...
    createWidget: (options: NavigatableWidgetOptions) => {
      const treeContainer = createBasicTreeContainer(container, TreeEditorWidget, TreeModelService, TreeNodeFactory);
